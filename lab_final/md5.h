...
// GPU v3: kernel-side padding (CPU strings → GPU hash)
int MD5HashBatch_GPU_Raw(const string* inputs, int n, bit32* results, float* kernel_ms = nullptr);

// GPU v4: hash passwords already on GPU (no H2D!)
...
// GPU generate exposure (from guessing.cu)
extern "C" void GetGPUPasswords(const char** out_buf, const int** out_lens, int* out_count);
extern "C" void ClearGPUPasswords();
