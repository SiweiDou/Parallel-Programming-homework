...
// Macros for device-compatible MD5 operations
...
// ---- GPU v1/v2 kernel: takes pre-padded 64-byte blocks ----
...
// ---- GPU v3 kernel: takes raw strings, does padding and MD5 internally ----
// raw_chars: concatenated raw strings (flat char array, max_len bytes each, padded with zeros)
// lengths: length of each raw string (int per password)
// n: number of passwords
// max_len: maximum string length in this batch
__global__ void MD5Kernel_Raw(const char* raw_chars, const int* lengths, bit32* results, int n, int max_len)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;

    // Point to this thread's raw string
    const char* my_str = raw_chars + tid * max_len;
    int len = lengths[tid];

    // Build padded 64-byte block in-kernel
    ...
    for (int k = 0; k < 8; ++k) {
        padded[56 + k] = (bit_len >> (k * 8)) & 0xFF;
    }

    // Decode 16 uint32 values
    bit32 x[16];
    for (int i = 0; i < 16; ++i) {
        x[i] = (padded[4 * i]) |
               (padded[4 * i + 1] << 8) |
...
// ---- Host-side: GPU v3 batch MD5 using kernel-side padding ----
// Sends raw strings (much smaller than 64B blocks) to GPU
int MD5HashBatch_GPU_Raw(const string* inputs, int n, bit32* results, float* kernel_ms = nullptr)
{
    ...
    // Allocate pinned memory for raw strings (flat array, max_len per string)
    char* h_raw;
    int* h_lengths;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        memcpy(h_raw + i * max_len, inputs[i].c_str(), h_lengths[i]);
    }
    ...
    MD5Kernel_Raw<<<grid_size, block_size>>>(d_raw, d_lengths, d_results, n, max_len);
    ...
    for (int i = 0; i < n; ++i) {
        results[i * 4 + 0] = h_results[i * 4 + 0];
        results[i * 4 + 1] = h_results[i * 4 + 1];
        results[i * 4 + 2] = h_results[i * 4 + 2];
        results[i * 4 + 3] = h_results[i * 4 + 3];
    }
...
// ---- Legacy GPU v2 (kept for reference) ----
static bool PadPassword(const string& input, unsigned char padded[64])
{
    int len = input.length();
    if (len > 55) return false;
    memcpy(padded, input.c_str(), len);
    padded[len] = 0x80;
    memset(padded + len + 1, 0, 56 - len - 1);
    unsigned long long bit_len = ((unsigned long long)len) * 8ULL;
    for (int k = 0; k < 8; ++k)
        padded[56 + k] = (bit_len >> (k * 8)) & 0xFF;
    return true;
...
// GPU v2 (kept for compatibility, no longer called in v3 path)
int MD5HashBatch_GPU(const string* inputs, int n, bit32* results, float* kernel_ms = nullptr)
{
    if (n == 0) return 0;
    // Delegate to raw version for simplicity in v3
