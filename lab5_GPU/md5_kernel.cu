...
__global__ void MD5Kernel(const unsigned char* padded_data, bit32* results, int n)
{
    ...
    // Byte-swap to match CPU MD5Hash output convention
...
int MD5HashBatch_GPU(const string* inputs, int n, bit32* results, float* kernel_ms = nullptr)
{
    ...
    for (int i = 0; i < valid_count; ++i) {
        ...
        results[orig_idx * 4 + 1] = h_results_gpu[i * 4 + 1];
        results[orig_idx * 4 + 2] = h_results_gpu[i * 4 + 2];
        results[orig_idx * 4 + 3] = h_results_gpu[i * 4 + 3];
    }
